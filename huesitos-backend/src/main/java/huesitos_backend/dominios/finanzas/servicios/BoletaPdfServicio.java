    ...
    public byte[] generarPdfBoleta(Long transaccionId) {
        ...
        try {
            ...
            Paragraph subheader = new Paragraph(
                    "R.U.C. N° 20123456789\nCalle de las Mascotas 123 - San Borja\nContacto: info@huesitos.com | (01) 456-7890",
                    subFont);
            ...
            tableDetalles.addCell(crearCeldaSinBorde("Boleta N°: B001-" + String.format("%06d", transaccion.getId()),
                    labelFont, valueFont));
            tableDetalles.addCell(crearCeldaSinBorde("Fecha Emisión: " + fechaStr, labelFont, valueFont));

            tableDetalles.addCell(
                    crearCeldaSinBorde("Cliente: " + transaccion.getCita().getMascota().getDueño().getNombreCompleto(),
                            labelFont, valueFont));
            tableDetalles.addCell(crearCeldaSinBorde("Medio de Pago: " + transaccion.getMedioPago().toString(),
                    labelFont, valueFont));

            tableDetalles.addCell(crearCeldaSinBorde("Mascota: " + transaccion.getCita().getMascota().getNombre() + " ("
                    + transaccion.getCita().getMascota().getEspecie() + ")", labelFont, valueFont));
            String pasarelaId = transaccion.getIdTransaccionPasarela() != null ? transaccion.getIdTransaccionPasarela()
                    : "Pago Presencial";
            ...
            Paragraph totalParagraph = new Paragraph("TOTAL PAGADO: S/. " + transaccion.getMonto().toString(),
                    totalFont);
            ...
            Paragraph footer = new Paragraph("¡Gracias por confiar en nosotros para cuidar de su mascota! 🐾",
                    footerFont);
