  ...
  return (
    <div className={`flex h-screen bg-slate-50 font-sans overflow-hidden ${selectionClass}`}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-50">
        {/* Header Superior Responsivo con Contraste Suave de Azul Cielo */}
        <header className="bg-sky-50/95 backdrop-blur-sm h-20 px-4 md:px-6 lg:px-8 flex justify-between items-center border-b border-sky-100/85 sticky top-0 z-30 shrink-0 transition-all">
          <div className="flex items-center gap-3 min-w-0">
            {/* Botón hamburguesa */}
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden p-2 rounded-lg text-sky-600 hover:bg-sky-100/80 hover:text-sky-800 transition-colors shrink-0"
              aria-label="Abrir menú"
            >
              <Menu size={20} />
            </button>
            <h1 className="text-base sm:text-lg md:text-xl font-black text-sky-950 tracking-tight truncate max-w-[140px] sm:max-w-xs md:max-w-none">
              {tituloHeader}
            </h1>
          </div>
          ...
          <div 
            ...
            className="flex items-center gap-2 bg-white hover:bg-sky-50 border border-sky-200/60 hover:border-sky-300 p-1.5 rounded-full md:pr-4 md:gap-3 shrink-0 cursor-pointer active:scale-95 transition-all select-none shadow-sm group"
            title="Ver mi perfil"
          >
            ...
            <span className="text-xs md:text-sm font-bold text-sky-900 group-hover:text-sky-950 transition-colors truncate max-w-[80px] sm:max-w-[120px] md:max-w-[200px]" title={correo}>
              {correo}
            </span>
          </div>
        </header>
