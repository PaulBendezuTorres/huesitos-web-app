  ...
  return (
    <div className={`flex h-screen bg-slate-50 font-sans overflow-hidden ${selectionClass}`}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-50">
        {/* Header Superior Responsivo con Contraste Premium Oscuro */}
        <header className="bg-slate-900 h-20 px-4 md:px-6 lg:px-8 flex justify-between items-center border-b border-slate-800/80 sticky top-0 z-30 shrink-0 transition-all">
          <div className="flex items-center gap-3 min-w-0">
            {/* Botón hamburguesa */}
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden p-2 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white transition-colors shrink-0"
          ...
          <div 
            ...
            className="flex items-center gap-2 bg-slate-950/60 hover:bg-slate-800/80 border border-slate-800 hover:border-slate-700 p-1.5 rounded-full md:pr-4 md:gap-3 shrink-0 cursor-pointer active:scale-95 transition-all select-none shadow-inner group"
            title="Ver mi perfil"
          >
            ...
            <span className="text-xs md:text-sm font-bold text-slate-300 group-hover:text-white transition-colors truncate max-w-[80px] sm:max-w-[120px] md:max-w-[200px]" title={correo}>
              {correo}
            </span>
          </div>
        </header>
