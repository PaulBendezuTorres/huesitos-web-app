...
const PlantillaTablero = ({
  ...
  return (
    <div className={`flex h-screen bg-slate-50 dark:bg-slate-900 font-sans overflow-hidden ${selectionClass}`}>
      ...
      {/* Barra Lateral Modular Dinámica — siempre oscura */}
      <BarraLateral
        rol={rol}
        correo={correo}
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-50 dark:bg-slate-900">
        {/* Header Superior */}
        <header className="bg-slate-700 h-20 px-4 md:px-6 lg:px-8 flex justify-between items-center border-b border-slate-600/60 sticky top-0 z-30 shrink-0 transition-all">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div 
              ...
              className="flex items-center gap-2 bg-slate-600/70 hover:bg-slate-600 border border-slate-500/50 hover:border-slate-400 p-1.5 rounded-full md:pr-4 md:gap-3 cursor-pointer active:scale-95 transition-all select-none shadow-sm group"
              title="Ver mi perfil"
            >
