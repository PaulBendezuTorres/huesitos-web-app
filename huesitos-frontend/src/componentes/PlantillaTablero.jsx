  ...
  return (
    <div className={`flex h-screen bg-slate-50 font-sans overflow-hidden ${selectionClass}`}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-50">
        {/* Header Superior Responsivo con Contraste Suave de Gris */}
        <header className="bg-slate-100/95 backdrop-blur-sm h-20 px-4 md:px-6 lg:px-8 flex justify-between items-center border-b border-slate-200/80 sticky top-0 z-30 shrink-0 transition-all">
          <div className="flex items-center gap-3 min-w-0">
            {/* Botón hamburguesa */}
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden p-2 rounded-lg text-slate-500 hover:bg-slate-200/60 hover:text-slate-800 transition-colors shrink-0"
          ...
          <div 
            ...
            className="flex items-center gap-2 bg-white hover:bg-slate-50/80 border border-slate-200 hover:border-slate-300 p-1.5 rounded-full md:pr-4 md:gap-3 shrink-0 cursor-pointer active:scale-95 transition-all select-none shadow-sm group"
            title="Ver mi perfil"
          >
