...
import { obtenerAlertasBajoStock, obtenerAlertasVencimientos } from '../services/finanzasService';

const InventarioCriticoWidget = () => {
    ...
    <>
      ...
      <div className="p-4 border-b border-slate-200 bg-white flex justify-between items-center shrink-0">
        ...
        <button
          ...
          disabled={loading}
          className="p-1.5 bg-white hover:bg-slate-50 border border-slate-200 shadow-sm rounded-lg text-xs font-bold text-slate-500 hover:text-sky-600 transition-all flex items-center justify-center"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto max-h-[220px] pr-1 text-left">
              {loading ? (
                <p className="text-xs text-slate-400 animate-pulse font-bold text-center py-4">Consultando stock...</p>
              ) : bajoStock.length === 0 ? (
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto max-h-[220px] pr-1 text-left">
              {loading ? (
                <p className="text-xs text-slate-400 animate-pulse font-bold text-center py-4">Analizando lotes...</p>
                ...
                vencimientos.map((lote) => (
                  <div key={lote.id} className="flex justify-between items-center p-2.5 rounded-xl bg-amber-50/50 border border-amber-150 text-xs">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400">Lote: {lote.numeroLote || lote.codigoLote || 'N/A'}</p>
                    </div>
...
export default InventarioCriticoWidget;
